...
TwampLiteSender::TwampLiteSender(const std::string& ip, uint16_t port): reflector_ip(ip), reflector_port(port) {
...
void TwampLiteSender::run(int num_packets = 3, int interval_ms = 10, int timeout_interval_ms = 1000) {
    ...
    for (int i = 0; i < num_packets; ++i) {
        ...
        TwampLitePacket pkt(seq, send_time, 0);
        ...
        TwampLitePacket resp = TwampLitePacket::deserialize(recv_buffer, len);

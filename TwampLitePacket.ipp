...
TwampLitePacket::TwampLitePacket(uint32_t seq, uint64_t sender_ts, uint64_t receiver_ts)
{
    sequence_number = seq;
...
//serialize definiton
std::vector<uint8_t> TwampLitePacket::serialize() const {
    //vector of 20 bytes
...
//derialize definiton
TwampLitePacket TwampLitePacket::deserialize(const uint8_t* data, size_t length) {
    uint32_t seq_net;
    ...
    return TwampLitePacket(seq, sender_ts, receiver_ts);
}